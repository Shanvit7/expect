...
describe("resolveTestTarget", () => {
  ...
  it("resolves branch targets against main", async () => {
    commandOutputs.set("git rev-parse --abbrev-ref HEAD", "feature-branch");
    commandOutputs.set("git rev-parse --verify main", "main-hash");
    commandOutputs.set("git diff main...HEAD --shortstat", "5 files changed, 14 insertions(+), 2 deletions(-)");
    commandOutputs.set("git diff --name-status main...HEAD", "M\tsrc/onboarding.ts\nA\tsrc/import.ts");
    commandOutputs.set(
      "git log --format=\"%H\u001f%h\u001f%s\" -n 10 main..HEAD",
  ...
  it("resolves commit targets from the selected commit hash", async () => {
    ...
    commandOutputs.set("git show --shortstat --format= deadbeef", "1 file changed, 8 insertions(+), 1 deletion(-)");
    commandOutputs.set("git diff-tree --no-commit-id --name-status -r deadbeef", "M\tsrc/onboarding.ts");
    commandOutputs.set(
